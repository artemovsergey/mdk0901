...
**Тема:** Развертывание и настройка Full-Stack приложения (.NET + React/Vite) на виртуальном частном сервере (VPS) с использованием Nginx как reverse proxy.
...
5.  Собрать production-версии Backend (ASP.NET Core Web API) и Frontend (React + Vite) приложений.
...
3.  **Nginx** — высокопроизводительный HTTP-сервер и обратный прокси (reverse proxy). Он может:
    *   Раздавать статические файлы (собранное React-приложение).
...
2.  **Подготовка приложения:**
    ...
    *   **Frontend:** Выполнить `npm ci` и `npm run build`. Скопировать содержимое папки `dist` на сервер в `/var/www/todo-app/`.
...
4.  **Настройка Nginx:**
    ...
    *   Добавить конфигурацию, которая проксирует запросы к API на порт 5000, а все остальные запросы отдаёт на статику React:
        ```nginx
        server {
            listen 80;
...
5.  **Проверка:** Открыть в браузере `http://your_server_ip`. Должно загрузиться React-приложение, которое может взаимодействовать с API.
...
*   **4 (Хорошо):**
    *   Приложение развернуто, но есть незначительные ошибки (например, проблемы с путями в React-приложении, не все функции API работают).
...
2.  Какова роль директивы `try_files` в конфигурации Nginx для React-приложения (SPA)?
...
## Задание
Развернуть приложение "To-Do List" на VPS с ОС Ubuntu 22.04. Backend - ASP.NET Core Web API, Frontend - React (Vite). Настроить Nginx как reverse proxy и запустить backend как системный сервис.
